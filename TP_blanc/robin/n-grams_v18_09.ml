...
let construire_n_grams_array_parallele (texte: string) (n: int) (nombre_threads: int): n_gram array =
  ...
  let per_thread_fun ((i, debut, fin): int * int * int): unit =
    resultat.(i) <- construire_n_grams_partiel resultat.(i) texte debut fin n
  in
  ...
  let threads = Array.init (nombre_threads - 1) (fun i ->
    ...
    (* Je ne semble pas avoir Thread installé dans cet environement
    Thread.create per_thread_fun (j, frontiere j, frontiere (j + 1))) *)
    per_thread_fun (j, frontiere j, frontiere (j + 1)))
  in
  per_thread_fun (0, 0, frontiere 1);
  (* Toujours le problème de Thread pas trouvé
...
let construire_n_grams_parallele (texte: string) (n: int) (nombre_threads: int) =
...
let test_construction_parallele (texte: string) (n: int) (nombre_threads: int) =
  ...
  est_equivalent_n_grams (construire_n_grams texte n)
                         (construire_n_grams_parallele texte n nombre_threads)
;;
